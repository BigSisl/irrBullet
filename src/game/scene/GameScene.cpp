...
void GameScene::onLoad(){
  ...
  //
	...
	scene::IMeshSceneNode *Node = device->getSceneManager()->addMeshSceneNode(device->getSceneManager()->getMesh("../media/terrainMain.b3d"),
                                        0, TERRAIN, core::vector3df(0.0f,0.0f,0.0f), core::vector3df(0.0f,0.0f,0.0f), core::vector3df(1.0f,1.0f,1.0f));
...
void GameScene::onTick(){
  ...
  scene::ISceneNode* selectedSceneNode =
    collMan->getSceneNodeAndCollisionPointFromRay(
                    ...
                    TERRAIN, // This ensures that only nodes that we have
  ...
  if(world->getKeys()->get(KEY_KEY_W)){
    //move forward
    character->move(core::vector3df(10000.0f * world->getFrameDeltaTime(),0.0f,0.0f));
  }
